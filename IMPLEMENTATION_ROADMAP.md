...
### 4.5 Coupon performance report (admin analytics) ✅ Done (2026-09-02)
- Backend: `GET /api/analytics/coupons?from&to` — per coupon: redemptions, discounted total, revenue attributable (orders with coupon vs. not), unique customers. `[Authorize(Roles = "Admin")]` (matches revenue-only rule you already apply). ✅ `AnalyticsRepository.GetCouponPerformanceAsync` aggregates `CouponUsage` in range (cancelled/returned orders excluded), plus a with-coupon vs without-coupon summary; endpoint in `AnalyticsController` (super-admin only).
